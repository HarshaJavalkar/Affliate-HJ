...
productApiObj.post(
  ...
  errorHandler(async (req, res) => {
    ...
    let product = await Affliate.findOne({ _id: prod_id });
...
productApiObj.post(
  "/getproducts",
  errorHandler(async (req, res) => {
    let products = await Affliate.find({
...
productApiObj.post(
  ...
  errorHandler(async (req, res) => {
    ...
    let product = await Affliate.updateOne(
...
productApiObj.post(
  ...
  errorHandler(async (req, res) => {
    ...
    let product = await Affliate.findOne(filter);
    res.send({ message: product });
  })
);
...
productApiObj.post(
  ...
  errorHandler(async (req, res) => {
    ...
    for (let i = 0; i < cartObj.length; i++) {
      if (cartObj.length) {
        let cartDel = await Affliate.findOne({ _id: cartObj[i]._id });
...
productApiObj.get(
  "/getAllAffliatesUsers",
  errorHandler(async (req, res) => {
    let productsTOUsers = await Affliate.find();
...
productApiObj.post(
  ...
  errorHandler(async (req, res) => {
    ...
    req.body.photo = req.file.path;
    let productObj = req.body;
    console.log("h",productObj)
    let affliate = await Affliate.findOne({ prod_id: req.body.prod_id });

    console.log("s",affliate)
    if (affliate == null) {
      let prodObj = new Affliate({
        active: productObj.active,
        creator: productObj.creator,
        prod_price: productObj.prod_price,
        prod_desc: productObj.prod_desc,
        ...
        photo: productObj.photo,
        type: productObj.type,
        affliateLink: productObj.affliateLink,
        overallRating: productObj.overallRating,
      });

      await prodObj.save();
      console.log("saved")
      res.send({ message: "Affliate created successfully" });
    } else {
      res.send({ message: "Affliate already exist" });
    }
  })
);
